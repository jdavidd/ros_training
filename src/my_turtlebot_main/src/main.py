...
class ControlTurtlebot(object):
    ...
    def init_direction_service_client(self, service_name = "/crash_direction_service_laser"):
...
control_turtlebot_object = ControlTurtlebot(goal_distance=8.0)
rate = rospy.Rate(10)
...
while not control_turtlebot_object.rec_odom_finished():
    ...
    if direction_to_go.success is False:
        control_turtlebot_object.move_turtlebot("forward")
    else:
